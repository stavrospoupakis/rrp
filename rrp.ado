...
tempvar ones 
tempname R2_first sigma_first sigma_second ALL XX ZX XZ ZZ beta V
...
qui reg yhat_RRP `varlist'  `if' `in'
...
mat `ZX' = `XZ''
mat `V'  = syminv(`XX') * ( `XX' * (`sigma_second'^2) + `XZ' * (`sigma_first'^2) * syminv(`ZZ') * `ZX' / `R2_first' / `R2_first'    ) * syminv(`XX') 
cap drop _est_*
ereturn post `beta' `V' 
